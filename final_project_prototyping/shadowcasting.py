...
def compute_fov(origin, is_blocking, mark_visible, max_perception_depth = 10000):
        ...
        def scan(row, iter):
            ...
            for tile in row.tiles():
                ...
                if is_floor(prev_tile) and is_wall(tile):
                    ...
                    if iter < max_perception_depth:
                        scan(next_row, iter+1)

                prev_tile = tile
            
            if is_floor(prev_tile) and iter < max_perception_depth:
                scan(row.next(), iter+1)

        
        first_row = Row(1, Fraction(-1), Fraction(1))
        scan(first_row, iter=0)
